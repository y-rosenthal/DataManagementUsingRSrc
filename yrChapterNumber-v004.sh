...
# URL: https://chatgpt.com/c/670953e9-bc5c-800b-8d27-60ed9eeb8258
# CHAT NAME:  "create yrChapterNumber.sh script")

* This script needs to be run anytime a new .qmd file is added or the chapter numbers change
...
grep -i 'yrChapterNumber' _quarto.yml | awk '!/^\s*#/' | awk '{$1=$1; print}' | while read -r line; do
  # Extract filename and yrChapterNumber
  filename=$(echo "$line" | grep -oP '^[^#]+' | xargs | sed 's/^-\s*//')
  yrChapterNumber=$(echo "$line" | sed -n 's/.*# yrChapterNumber\s\+\(.*\S\).*/\1/p' | sed 's/$/. /')

  # Ensure that the extracted filename is actually a .qmd file
  if [[ ! "$filename" =~ \.qmd$ ]]; then
