# <yrChapterNumber>17. </yrChapterNumber> (a) word boundaries `\b` (b) PATTERN1|PATTERN2 (c) paste0 to build long patterns
...
- You can use `\b` before a pattern, e.g. `\bSOME_PATTERN` 
...
- You can use `\bSOME_PATTERN\b` to mean that SOME_PATTERN must match
  an entire word (i.e. there is word break both before and after the pattern).

::: {.callout-note}
A "word boundary" is not a particular character such as a 
space or comma, but rather is a position in the text. 
:::

::: {.callout-note}
A "word" may include letters, digits or underscores (see the discussion 
of the `\w` and `\W` metacharacters above.)
:::

::: {.callout-note}
Some students get confused between `\bSOME_PATTERB` and `^SOME_PATTERN` (caret). 
