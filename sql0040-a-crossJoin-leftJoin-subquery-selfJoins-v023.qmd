...
## COALESCE(item1, item2, ...) returns first item that is not NULL

The SQL coalesce function is a non-aggregate function. It takes multiple
values as arguments and returns the first argument that is not NULL. If
all arguments are NULL then COALESCE returns NULL. 
This offers a very simple way to replace NULLs in the output with 
something that is more user friendly. For example, above we showed
how you could use the LEFT JOIN to show publishers who have not published
any books. The following shows that Tenterhooks press did not publish
any titles.
...
Instead of displaying NULL for the title_name for Tenterhooks press, it may
be preferable to display a message indiating that this publisher has
no associated titles. That is done in the following revised command
that uses the coalesce function:
...
# refers to the authors.au_id table even though the from clause in the
...
#---------------------------------------------------------------------------.
# EXAMINING HOW THE CORRELATED SUBQUERY WORKS
#
# When processing a correlated subquery, SQL will "under the covers"
# process a separate inner query for every row that is processed in the 
...
#------------------------------------------------------------------------------.
# You CANNOT simply add the number of titles to the version with the subquery as
...
#------------------------------------------------------------------.
# You CAN get the number of books into the output by using yet
# another subquery in the SELECT clause.
