...
In this assignment, you'll practice using regular expressions to identify and clean messy data. The dataset below contains employee expense records with various inconsistencies in formatting.
For example:

**Inconsistencies with employee names**

- some names are in "first last" format (e.g. "Sue Smith")   
  and some names are in "last, first" format (e.g. "Brown, Tom")

- Some names are Capitalized Properly, and some names aren't (e.g. "schwartz, joe")
  e.g.   
  
- Numerous other formats, some are missing first names, some use periods, some don't,
  etc. 

**Inconsistencies with dates, amounts, category columns**

The other columns have similar inconsistencies in formatting the data

- date column: The following are all different forms of the exact same date.   
  1/19/2023 , 2023-10-19 , 19/01/23
  There are yet other forms. 

- amount column:  Some values have $ signs, some don't, some have commas (e.g. 1,234.56)
  some don't, some are missing the cents (e.g. "75"), some are written in words
  (e.g. "five hundred")
  
- category column: the same categories are written differently 
  (e.g. "food" vs "FOOD", "office supplies" vs "Office suppl." vs "OFFICE STUFF"
  etc.
  
- comments column: blanks are expressed differently: NA vs "NA" vs "---".
