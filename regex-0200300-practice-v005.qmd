...
Use regular expressions to r
emove all whitespace that appears before or after an entry in the 
...
rowsWithWhitespace = expenses$employee = sub("^\\s+|\\s+$", "", expenses$employee)
rowsWithWhitespace
...
# Show the rows that were changed in the old dataframe and the new dataframe

data.frame ( oldEntries = expenses$employee[rowsWithWhitespace],
             fixedEntries = newExpenses$employee[rowsWithWhitespace])
