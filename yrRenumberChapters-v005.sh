...
while IFS= read -r line; do
    ...
    elif [[ $line =~ .*#[[:space:]]*yrChapterNumber[[:space:]]+[0-9]+[^[:space:]]*[[:space:]]*$ ]]; then
        # Extract current number
        current_num=$(echo "$line" | grep -oE '[0-9]+[^[:space:]]*[[:space:]]*$')
        current_num=${current_num%% *} # Remove trailing whitespace
        
        # Preserve everything up to the last occurrence of "# yrChapterNumber"
        prefix=$(echo "$line" | sed -E 's/#[[:space:]]*yrChapterNumber[[:space:]]+[0-9]+[^[:space:]]*[[:space:]]*$//')
