...
while IFS= read -r line; do
    # Check if the line matches the chapter number pattern, allowing for trailing content
    if [[ $line =~ ^([[:space:]]*-[[:space:]]*[^#]*)?[[:space:]]*#[[:space:]]*yrChapterNumber[[:space:]]+[0-9]+.*$ ]]; then
        # Extract the leading part (if any) and preserve it
