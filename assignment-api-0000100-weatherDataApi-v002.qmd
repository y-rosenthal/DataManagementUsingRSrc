...
- Feb. 28, 2025 - Modifications:
  ...
  * To see exactly which lines changed (you really don't have to) you can
    ...
    (for this current version of this webpage). The left side of the screen
    shows the contents of the original .qmd file and the right side the 
    contents of the .qmd file that generated this version of the page.
...
# <yrChapterNumber>22. </yrChapterNumber> assignment - weather API
...
but you must understand the code before submitting it (you should do your
own research about any functions that we didn't cover in class).
...
Use the yrWebSnap chrome extension to take a snapshot of the HTML page
of the conversations you had with the AI. Do NOT submit 
images (i.e. screenshots) of the AI conversation. Rather, I want an
HTML file that was produced by the yrWebSnap Chrome extension, so
that I can copy/paste the text from your conversations. You can't easily
copy/paste text from an image.
...
#### function getForecastDf() {#function-getForecaseId}
...
Your function should check that the arguments make sense. Both latitude
and longitude should be numeric vectors of length 1 (i.e. just one number
for each argument). If the arguments don't follow these rules then the 
function should stop with an appropriate error
message - i.e. call stop( SOME_ERROR_MESSAGE )


##### (b) Call the API and return a dataframe with the data

The function should call the API to get the weather data.
The function should return a non-nested dataframe (i.e. a "normal" dataframe 
...
In addition, to the data that is in the JSON, your dataframe should include
the following columns. These extra columns will have the exact same data 
for each row in the dataframe and should be the first three columns in 
the dataframe:
...
#### function createForecastFiles() {#function-createForecastFiles}
...
* latitude, longitude, sleepSeconds and numberOfApiCalls should all be
  numeric vectors whose length is 1.
...
The `createForecastFiles` function should continuously call your
`getForecastDf` function (that was described above) in a loop. 
The loop should iterate (i.e. "go around")
