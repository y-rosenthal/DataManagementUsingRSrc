...
# Regex: one or more spaces, then (Jr or Sr or III or II or IV), then optional spaces at end.
suffix_regex <- paste0(
  "\\s+",           # one or more spaces before suffix
  "(Jr\\.?|Sr\\.?|III|II|IV)",  # suffix (optional period after Jr/Sr)
  "\\s*$"           # optional spaces at end of string
)
has_suffix <- function(namePart) {
  grepl(suffix_regex, namePart, ignore.case = TRUE)
}
...
extract_suffix <- function(namePart) {
  ...
  suffix <- sub(paste0(".*", suffix_regex), "\\1", namePart, ignore.case = TRUE)
  nameWithoutSuffix <- sub(suffix_regex, "", namePart, ignore.case = TRUE)
