...
## Restrictions on SELECT clauses that contain aggregate functions, (eg. avg, sum, min, max, count)

**If a SELECT clause includes aggregate functions then the SELECT should NOT**
**include non-aggregated columns, unless those columns are part of the**
**GROUP BY clause.**

*(The rest of this section explains this "rule")*
...
- if there is no GROUP BY, then only one row is returned, (see example below)

- if the statement contains a GROUP BY then one row is returned per group of 
  rows in the table. (see the [GROUP BY clause](#yrGroupBy) above)

For example, in the following statement, the SELECT clause includes aggregate 
functions but does NOT contain a group by. Therefore only one row is returned.
The next example includes a GROUP BY which causes more than one row to 
be returned. 
...
the following includes a GROUP BY. Therefore
...
from all rows in the table to the aggregate function. If a select has a GROUP BY
then the aggregate functions are called once for each group of rows. 

A SELECT clause that includes aggregate functions which also includes 
non-aggregated values that are not part of the GROUP BY is problematic.
Because of the aggregate function(s) only a single row will be returned. 
However, the non-aggregated values could have multiple rows. This causes 
a conflict - should only one row be returned (because of the aggregate 
functions) of should more than one row be returned (because of the non-aggregated data)?
...
For example, *DO NOT DO THE FOLLOWING*. You will NOT get an error, but
the output WILL BE MISLEADING.

The following SELECT uses aggreagate functions and INCORRECTLY also
includes region which is NOT part of the GROUP BY. It appears that the output
...
The following has the same problem but for a query that doesn't have 
any group by.
